...
<body>
    ...
    try {
        ...
        async function deleteComponent(id) {
            const comp = components.find(c => c.IDCOMPONENTE == id);
            if (!comp) return;
            if (!(await stoktechConfirm({ titulo: 'Excluir componente', mensagem: `Você tem certeza que deseja excluir "${comp.NOME}"?`, textoConfirmar: 'Excluir' }))) return;
            const fd = new FormData(); fd.append('acao', 'excluir_componente'); fd.append('id', id);
            try {
                const res = await fetch('gerenciar_componentes.php', { method: 'POST', body: fd });
                const data = await res.json();
                if (data.sucesso) { showNotification(data.mensagem); renderComponents(); } else { stoktechToast(data.mensagem, 'erro'); }
