...
if ($acao === 'excluir_componente') {
    ...
    try {
        ...
        ");
        $stmtUso->execute([$id]);
        $uso = (int)$stmtUso->fetch()['total'];

        if ($uso > 0) {
            echo json_encode([
                'sucesso' => false,
                'mensagem' => 'Não é possível excluir este componente porque ele já está vinculado a empréstimos. Para manter o histórico correto, apenas deixe o estoque como 0.'
            ], JSON_UNESCAPED_UNICODE);
            exit;
        }
        ...
        if (!empty($comp['IMAGEM']) && is_string($comp['IMAGEM']) && file_exists($comp['IMAGEM'])) {
            unlink($comp['IMAGEM']);
        }
        ...
        echo json_encode([
            'sucesso' => false,
            'mensagem' => 'Erro ao excluir componente: ' . $e->getMessage()
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }
}
